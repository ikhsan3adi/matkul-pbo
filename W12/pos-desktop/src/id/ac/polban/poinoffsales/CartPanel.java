...
public class CartPanel extends JPanel {
  ...
  private void generateReceipt() {
    ...
    for (int i = 0; i < rowCount; i++) {
      ...
      int price = (int) cartTableModel.getValueAt(i, 3);

      String line = String.format("%-20s%n      %d x %s%n", name, qty, currencyFormatter.format(price));

      receipt.append(line);
    }

    receipt.append("---------------------------------------\n");
    receipt.append(String.format("TOTAL: %s%n", currencyFormatter.format(totalAmount)));
    receipt.append(String.format("POIN DIDAPAT: %d%n%n", totalAmount / 10000));
